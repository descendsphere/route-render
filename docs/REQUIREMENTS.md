...
*   **FR-1.6:** The user shall be able to pre-configure application settings (e.g., camera, speed, smoothing factor) via URL query parameters.
...
*   **FR-4.6:** The application shall provide multiple camera strategies (e.g., Third-Person, Overhead Orbit).
*   **FR-4.7:** The user shall be able to manually adjust the smoothing factor for live metrics to balance responsiveness and visual noise.
*   **FR-4.8:** When the cinematic tour reaches its end, the application shall display the final accumulated statistics for the route.

### FR-5: Route Intelligence and Enhancement

*   **FR-5.1:** The application shall automatically identify and display relevant Points of Interest (POIs) near the route by querying the OpenStreetMap Overpass API, with icons and labels clamped to the ground.
...
*   **FR-5.3:** The application shall calculate and display key statistics for the route, including total distance and total elevation gain, within a consolidated, compact, and responsive Heads-Up Display (HUD) that unifies controls and stats, and adapts to screen size.
...
*   **NFR-5.1:** The JavaScript code must be written in a modular, component-based architecture. Each module shall have a single, clear responsibility (separation of concerns). A centralized, reactive settings manager is used to manage application state and decouple UI from business logic.
